# Lesson 3: Gateway & Channels (~30 min)
...
See: [Minimal config](https://docs.openclaw.ai/gateway/configuration#minimal-config-recommended-starting-point)
...
See: [channels.whatsapp](https://docs.openclaw.ai/gateway/configuration#channels-whatsapp-allowfrom), [Group chat mention gating](https://docs.openclaw.ai/gateway/configuration#group-chat-mention-gating-agents-list-groupchat--messages-groupchat)

### Session management

- Per-sender scope; `mainKey` (default `main`); DMs collapse to shared "main" session; groups get separate session keys.

See: [Session Management](https://docs.openclaw.ai/concepts/session), [Sessions](https://docs.openclaw.ai/concepts/sessions)

### Streaming and chunking

- Block streaming; channel-specific chunk limits and coalesce.

See: [Streaming and Chunking](https://docs.openclaw.ai/concepts/streaming)

### Slash commands

- `/new`, `/reset`, etc. Standalone message; leading `/`.
